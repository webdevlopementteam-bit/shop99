const MostSellingProduct = require("../models/mostSellingProductModel");
...
/* ================= ADD ================= */
exports.addMostSellingProducts = async (req, res) => {
  try {
    ...
    for (const id of productIds) {
      const exists = await MostSellingProduct.findOne({
        where: { product_id: id },
      });
    ...
    res.json({ message: "Most selling products added", data: created });
  } catch (error) {
...
/* ================= DELETE ================= */
exports.deleteMostSellingProduct = async (req, res) => {
  try {
    const { id } = req.params;

    await MostSellingProduct.destroy({
      where: { id },
    });
