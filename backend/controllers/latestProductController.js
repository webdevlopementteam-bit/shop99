const LatestProduct = require("../models/latestProductModel");
...
/* ================= ADD ================= */
exports.addLatestProducts = async (req, res) => {
  try {
    ...
    for (let id of productIds) {
      const exists = await LatestProduct.findOne({
        where: { product_id: id },
      });

      if (!exists) {
        const item = await LatestProduct.create({ product_id: id });
        created.push(item);
      }
    }

    res.json({ message: "Latest products added", data: created });
  } catch (error) {
...
exports.getLatestProducts = async (req, res) => {
  try {
    const data = await LatestProduct.findAll({
      include: [
        {
          model: Product,
          attributes: [
